...
import { getAuth, onAuthStateChanged, signOut, setPersistence, browserLocalPersistence, signInWithEmailAndPassword } from 'firebase/auth';
import { getFirestore, doc, setDoc, onSnapshot, updateDoc, collection, addDoc, getDocs, query, limit, orderBy, getDoc, arrayUnion, setLogLevel } from 'firebase/firestore';
import { Calendar, CheckCircle, BarChart2, Target, Clock, TrendingUp, Lightbulb, Home, FileText, BrainCircuit, Loader2, ArrowLeft, ArrowRight, Flag, XCircle, Sparkles, BookOpen, Trash2, CheckSquare } from 'lucide-react';
...
// --- INÍCIO: Configuração do Firebase e da API Gemini ---
...
const GEMINI_MODEL = "gemini-1.5-flash"; // Modelo mais estável
// --- FIM: Configuração ---
    ...
    const handleGenerateStudy = async () => { 
        ...
        if (topic.toLowerCase().includes("revisão")) {
            ...
            if (genOptions.questoes) {
                ...
                prompt += `### Gabarito Comentado\n[Apresente o gabarito das questões e, para cada uma, forneça um comentário explicando por que a alternativa correta está certa e as outras erradas.]`;
            }
        }
...
function ResultsView({ simulationData, onReview, onGoHome, userId }) {
    const { questions, userAnswers, analysisResult: cachedAnalysis, explanations: cachedExplanations } = simulationData;
    const [isAnalyzing, setIsAnalyzing] = useState(false);
    const [analysisResult, setAnalysisResult] = useState(cachedAnalysis || "");
    ...
    const handleAnalyzePerformance = async () => {
        ...
        // VERIFICA SE A ANÁLISE JÁ EXISTE NO FIRESTORE
        if (cachedAnalysis) {
        ...
        try {
            ...
            if (result.candidates && result.candidates[0]?.content?.parts?.[0]?.text) { 
                ...
                const simDocRef = doc(db, 'artifacts', appId, 'users', userId, 'simulations', simulationData.id);
...
function ReviewView({ simulationData, onGoHome, onBackToResults, userId }) {
    const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
    const [explanation, setExplanation] = useState("");
    const [isExplaining, setIsExplaining] = useState(false);
    const [explanationError, setExplanationError] = useState("");

    const { questions, userAnswers, explanations: cachedExplanations } = simulationData;
    ...
    useEffect(() => {
        setExplanation(cachedExplanations?.[currentQuestion.firestoreId] || "");
        setExplanationError("");
    }, [currentQuestionIndex, cachedExplanations]);
    ...
    const handleExplainQuestion = async () => {
        ...
        // VERIFICA SE A EXPLICAÇÃO JÁ EXISTE NO FIRESTORE
        const cachedExplanation = cachedExplanations?.[currentQuestion.firestoreId];
        ...
        **Sua Tarefa:**
        ...
        try { 
            ...
            if (result.candidates && result.candidates[0]?.content?.parts?.[0]?.text) { 
                ...
                const simDocRef = doc(db, 'artifacts', appId, 'users', userId, 'simulations', simulationData.id);
                await updateDoc(simDocRef, {
                    [`explanations.${currentQuestion.firestoreId}`]: formattedText
                });
            } else { 
                throw new Error("A resposta da API está vazia ou em formato inesperado."); 
            } 
    ...
    const isExplanationAvailable = useMemo(() => {
        return !!cachedExplanations?.[currentQuestion.firestoreId];
    }, [cachedExplanations, currentQuestion.firestoreId]);
        ...
        <div className="bg-white p-6 rounded-lg shadow-md max-w-4xl mx-auto">
            ...
            <div className="bg-gray-50 p-6 rounded-lg">
                ...
                <div className="mt-6 pt-4 border-t">
                    <button onClick={() => handleExplainQuestion(currentQuestion.firestoreId)} disabled={isExplaining || isExplanationAvailable} className="flex items-center justify-center mx-auto px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50">
                        {isExplaining ? <><Loader2 className="animate-spin mr-2" size={16} /> Carregando explicação...</> : <><Lightbulb className="mr-2" size={16} /> ✨ {isExplanationAvailable ? 'Explicação Gerada' : 'Me Explique Melhor'}</>}
                    </button>
...
function SimuladorOAB({ userId, pastSimulations, loadingHistory }) {
    ...
    const handleStartSimulation = async () => {
        ...
        try {
            ...
            console.error("Falha detalhada ao gerar o simulado:", err);
            setError(err.message || "Não foi possível carregar as questões do simulador.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const saveSimulationResults = async (results) => {
        ...
        try {
            ...
            console.error("Erro ao salvar resultado completo do simulador:", error);
        }
    };
